...
export class ProjectsService {
  ...
  async create(workspaceId: string, createProjectDto: CreateProjectDto, userId: string) {
    ...
    console.log('[ProjectsService] createProjectDto.collaborative_data:', createProjectDto.collaborative_data);
    console.log('[ProjectsService] Stringified collaborative_data:', projectData.collaborative_data);
    ...
    console.log('[ProjectsService] Project inserted - collaborative_data TYPE:', typeof project.collaborative_data);
    console.log('[ProjectsService] Project inserted - collaborative_data VALUE:', project.collaborative_data);
    console.log('[ProjectsService] =====================================');

    // Parse collaborative_data to get project members
    let collaborativeData = createProjectDto.collaborative_data || {};
    const projectMembers: string[] = collaborativeData.default_assignee_ids || collaborativeData.default_assignees || [];
    ...
    try {
      ...
      const defaultAssignees = collaborativeData.default_assignees || collaborativeData.default_assignee_ids || [];
      console.log('[ProjectsService] Default assignees extracted:', defaultAssignees);
      console.log('[ProjectsService] Checked fields: default_assignees =', collaborativeData.default_assignees, ', default_assignee_ids =', collaborativeData.default_assignee_ids);
      ...
      if (Array.isArray(defaultAssignees) && defaultAssignees.length > 0) {
        console.log('[ProjectsService] Sending notifications to default assignees:', defaultAssignees);
  ...
  async findAll(workspaceId: string, userId: string, filters?: { status?: string; type?: string }) {
    ...
    const workspaceMembershipResult = await this.db.table('workspace_members')
      .select('*')
      .where('workspace_id', '=', workspaceId)
      .where('user_id', '=', userId)
      .execute();

    const workspaceMembershipData = Array.isArray(workspaceMembershipResult.data) ? workspaceMembershipResult.data : [];
    ...
    const projectMembershipsResult = await this.db.table('project_members')
      .select('*')
      .where('user_id', '=', userId)
      .execute();

    const projectMemberships = Array.isArray(projectMembershipsResult.data) ? projectMembershipsResult.data : [];
    ...
    const allProjectsResult = await this.db.table('projects')
      .select('*')
      .execute();
    ...
    let projects = allProjectsData.filter(p =>
    ...
    const enrichedProjects = await Promise.all(
      projects.map(async (project) => {
        let parsedAttachments = typeof project.attachments === 'string'
          ? JSON.parse(project.attachments)
          : project.attachments;

        const parsedCollaborativeData = typeof project.collaborative_data === 'string'
        ...
        if ((!parsedAttachments || this.isEmptyAttachments(parsedAttachments)) && parsedCollaborativeData?.attachments) {
        ...
        return {
          ...project,
          kanban_stages: typeof project.kanban_stages === 'string'
  ...
  async update(id: string, updateProjectDto: UpdateProjectDto, userId: string) {
    ...
    try {
      ...
      const oldCollaborativeData = typeof project.collaborative_data === 'string'
        ? JSON.parse(project.collaborative_data)
        : project.collaborative_data || {};
      const oldAssignees: string[] = oldCollaborativeData.default_assignees || oldCollaborativeData.default_assignee_ids || [];

      // Get new assignees from update DTO - support both field names
      const newCollaborativeData = updateProjectDto.collaborative_data || {};
      const newAssignees: string[] = newCollaborativeData.default_assignees || newCollaborativeData.default_assignee_ids || [];
      ...
      if (updateProjectDto.collaborative_data?.default_assignees) {
        ...
        for (const assigneeId of addedAssignees) {
          if (assigneeId !== userId) {
            console.log(`[ProjectsService] Sending notification to newly added assignee: ${assigneeId}`);
            ...
            console.log(`[ProjectsService] Notification sent to newly added assignee: ${assigneeId}`);
          }
        }
        ...
        for (const assigneeId of removedAssignees) {
          if (assigneeId !== userId) {
            console.log(`[ProjectsService] Sending notification to removed assignee: ${assigneeId}`);
    ...
    if (this.entityEventIntegration) {
      try {
        this.entityEventIntegration.emitProjectUpdated(project.workspace_id, updatedProject, project, userId);
      } catch (error) {
  ...
  async remove(id: string, userId: string) {
    ...
    if (taskIds.length > 0) {
      ...
      const commentsResult = await this.db.table('task_comments')
        .select('*')
        .execute();
      ...
      const dependenciesResult = await this.db.table('task_dependencies')
        .select('*')
        .execute();
      const allDependencies = Array.isArray(dependenciesResult.data) ? dependenciesResult.data : [];
      const taskDependencies = allDependencies.filter(d =>
        taskIds.includes(d.task_id) || taskIds.includes(d.depends_on_task_id)
      );
      ...
      try {
        const assigneesResult = await this.db.table('task_assignees')
          .select('*')
          .execute();
      ...
      if (assigneeId !== userId) {
        try {
          await this.notificationsService.sendNotification({
            ...
            priority: (task.priority === 'high' || task.priority === 'urgent' ? 'high' : 'normal') as any,
  ...
  async getTasks(projectId: string, userId: string, sprintId?: string, status?: string) {
    ...
    const allTasksResult = await this.db.table('tasks')
      .select('*')
      .execute();
    ...
    return await Promise.all(tasks.map(async (task) => {
      const parsedAssignedTo = typeof task.assigned_to === 'string' ? JSON.parse(task.assigned_to) : task.assigned_to;
      ...
      if (task.updated_by) {
        try {
          ...
          if (userProfile) {
            ...
            updatedByUser = {
              id: userProfile.id,
              name: metadata.name || (userProfile as any).fullName || userProfile.name || userProfile.username || userProfile.email || 'Unknown User',
      ...
      return {
        ...
        collaborative_data: typeof task.collaborative_data === 'string' ? JSON.parse(task.collaborative_data) : task.collaborative_data,
        // Per-task custom fields (array of { id, name, fieldType, value, options? })
        custom_fields: typeof task.custom_fields === 'string' ? JSON.parse(task.custom_fields) : (task.custom_fields || []),
  ...
  async getTask(taskId: string, userId: string) {
    const taskQueryResult = await this.db.table('tasks')
      .select('*')
      .where('id', '=', taskId)
      .limit(1)
      .execute();
  ...
  async updateTask(taskId: string, updateTaskDto: UpdateTaskDto, userId: string) {
    ...
    const kanbanStages = typeof project.kanban_stages === 'string'
      ? JSON.parse(project.kanban_stages)
      : project.kanban_stages;

    // Find the last stage (completed stage) - the one with highest order
    const lastStage = kanbanStages && kanbanStages.length > 0
      ? kanbanStages.reduce((max: any, stage: any) => stage.order > max.order ? stage : max, kanbanStages[0])
      : null;
    ...
    const isBeingCompleted = updateTaskDto.status === completedStageId && task.status !== completedStageId;
    if (isBeingCompleted) {
    ...
    if (updateTaskDto.status && updateTaskDto.status !== completedStageId && task.status === completedStageId) {
    ...
    if (this.entityEventIntegration) {
      try {
        this.entityEventIntegration.emitTaskUpdated(project.workspace_id, updatedTask, task, userId);
      } catch (error) {
    ...
    try {
      ...
      if (isBeingCompleted && task.reporter_team_member_id && task.reporter_team_member_id !== userId) {
        await this.notificationsService.sendNotification({
  ...
  async assignTask(taskId: string, assigneeId: string, userId: string) {
    ...
    const existingAssigneesResult = await this.db.table('task_assignees')
      .select('*')
      .where('task_id', '=', taskId)
      .execute();

    const existingAssignees = Array.isArray(existingAssigneesResult.data) ? existingAssigneesResult.data : [];
  ...
  async getTaskComments(taskId: string, userId: string) {
    ...
    const commentsQueryResult = await this.db.table('task_comments')
      .select('*')
      .where('task_id', '=', taskId)
  ...
  async createTaskComment(taskId: string, createTaskCommentDto: CreateTaskCommentDto, userId: string) {
    await this.getTask(taskId, userId);
  ...
  async updateTaskComment(commentId: string, updateTaskCommentDto: UpdateTaskCommentDto, userId: string) {
    const commentQueryResult = await this.db.table('task_comments')
      .select('*')
      .where('id', '=', commentId)
      .limit(1)
      .execute();
  ...
  async getSprints(projectId: string, userId: string) {
    ...
    const sprintsQueryResult = await this.db.table('sprints')
      .select('*')
  ...
  private async getProjectWithAccess(projectId: string, userId: string) {
    const projectQueryResult = await this.db.table('projects')
      .select('*')
      .where('id', '=', projectId)
      .limit(1)
      .execute();
  ...
  async addProjectMember(projectId: string, memberId: string, userId: string) {
    ...
    const existingMemberData = Array.isArray(existingMemberResult.data) ? existingMemberResult.data : [];
    ...
    try {
      console.log(`[ProjectsService] Sending project member addition notification to user: ${memberId}`);
      ...
      console.log(`[ProjectsService] Project member addition notification sent to user: ${memberId}`);
    } catch (error) {
  ...
  async removeProjectMember(projectId: string, memberId: string, userId: string) {
    ...
    const memberQueryResult = await this.db.table('project_members')
      .select('*')
      .where('project_id', '=', projectId)
      .where('user_id', '=', memberId)
      .execute();
  ...
  async getProjectMembers(projectId: string, userId: string) {
    ...
    console.log('[ProjectsService] Members:', members.map(m => ({ user_id: m.user_id, role: m.role })));
    console.log('[ProjectsService] =====================================');
    ...
    if (attachments.file_attachment && Array.isArray(attachments.file_attachment)) {
      for (const fileId of attachments.file_attachment) {
        try {
          ...
          console.warn(`[ProjectsService] Could not fetch file attachment ${fileId}:`, error.message);
          enriched.file_attachment.push({ id: fileId, name: 'Unknown file' });
        }
      }
    }
    ...
    if (attachments.note_attachment && Array.isArray(attachments.note_attachment)) {
      for (const linkedNoteId of attachments.note_attachment) {
        try {
          ...
          console.warn(`[ProjectsService] Could not fetch note attachment ${linkedNoteId}:`, error.message);
          enriched.note_attachment.push({ id: linkedNoteId, title: 'Unknown note', icon: '📝' });
        }
      }
    }
    ...
    if (attachments.event_attachment && Array.isArray(attachments.event_attachment)) {
      for (const eventId of attachments.event_attachment) {
        try {
          ...
          console.warn(`[ProjectsService] Could not fetch event attachment ${eventId}:`, error.message);
          enriched.event_attachment.push({ id: eventId, title: 'Unknown event' });
        }
      }
    }
  ...
  async getCustomFields(projectId: string, userId: string) {
    ...
    const result = await this.db.table('task_custom_field_definitions')
      .select('*')
      .where('project_id', '=', projectId)
    ...
    return fields.map(field => ({
      ...
      defaultValue: typeof field.default_value === 'string' ? JSON.parse(field.default_value) : field.default_value,
  ...
  async createCustomField(projectId: string, createDto: any, userId: string) {
    ...
    const existingFieldsResult = await this.db.table('task_custom_field_definitions')
      .select('sort_order')
      .where('project_id', '=', projectId)
    ...
    const existingFields = Array.isArray(existingFieldsResult.data) ? existingFieldsResult.data : [];
    ...
    return {
      ...
      description: field.description,
      options: typeof field.options === 'string' ? JSON.parse(field.options) : field.options,
      defaultValue: field.default_value ? (typeof field.default_value === 'string' ? JSON.parse(field.default_value) : field.default_value) : null,
      isRequired: field.is_required,
      isVisible: field.is_visible,
      sortOrder: field.sort_order,
      settings: typeof field.settings === 'string' ? JSON.parse(field.settings) : field.settings,
  ...
  async updateCustomField(fieldId: string, updateDto: any, userId: string) {
    // Get the field first
    const fieldResult = await this.db.table('task_custom_field_definitions')
      .select('*')
      .where('id', '=', fieldId)
      .limit(1)
      .execute();
    ...
    if (updateDto.defaultValue !== undefined) updateData.default_value = JSON.stringify(updateDto.defaultValue);
    ...
    return {
      ...
      defaultValue: updatedField.default_value ? (typeof updatedField.default_value === 'string' ? JSON.parse(updatedField.default_value) : updatedField.default_value) : null,
      isRequired: updatedField.is_required,
      isVisible: updatedField.is_visible,
      sortOrder: updatedField.sort_order,
      settings: typeof updatedField.settings === 'string' ? JSON.parse(updatedField.settings) : updatedField.settings,
   ...
   */
  async addSelectOption(fieldId: string, optionData: { label: string; color?: string }, userId: string) {
    // Get the field first
    const fieldResult = await this.db.table('task_custom_field_definitions')
      .select('*')
      .where('id', '=', fieldId)
    ...
    // Get current options
    const currentOptions = typeof existingField.options === 'string'
