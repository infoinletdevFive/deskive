...
export class GoogleSheetsOAuthService {
   ...
   */
  decodeState(state: string): { userId: string; workspaceId: string; returnUrl?: string; timestamp: number; service: string } {
    try {
  ...
  getAuthorizationUrl(userId: string, workspaceId: string, returnUrl?: string): { authorizationUrl: string; state: string } {
    ...
    this.logger.log(`Generated OAuth URL for Google Sheets - user ${userId} in workspace ${workspaceId}`);
