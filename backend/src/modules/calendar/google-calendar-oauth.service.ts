...
export class GoogleCalendarOAuthService {
  ...
  private getClientCredentials() {
    ...
    this.logger.log(`Google OAuth credentials check - clientId: ${clientId ? 'set' : 'NOT SET'}, clientSecret: ${clientSecret ? 'set' : 'NOT SET'}`);
   ...
   */
  decodeState(state: string): { service: string; userId: string; workspaceId: string; returnUrl?: string; timestamp: number } {
    try {
